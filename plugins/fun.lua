...
local function get_weather(location)
	...
	local temp = 'دمای شهر '..city..' هم اکنون '..weather.main.temp..' درجه سانتی گراد می باشد 😐❤️'
...
local function calc(exp)
	...
	text = nil
	if c == 200 then
    text = 'حاصل جواب برابر است با 😐❤️ => '..b..''
	elseif c == 400 then
		text = b
	else
...
function run(msg, matches) 
	if matches[1]:lower() == "calc" and matches[2] or matches[1]:lower() == "ماشین حساب" and matches[2] then 
...
--------------------------------
	if matches[1]:lower() == 'اذان' or matches[1] == 'azan' then
		if matches[2] then
			city = matches[2]
...
--------------------------------
	if matches[1]:lower() == 'tophoto' and msg.reply_id or matches[1]:lower() == 'تبدیل به عکس' and msg.reply_id then
		function tophoto(arg, data)
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_استیکر یافت نشد 😐❤️_', 1, 'md')
				end
			end
...
--------------------------------
	if matches[1]:lower() == 'tosticker' and msg.reply_id or matches[1]:lower() == 'تبدیل به استیکر' and msg.reply_id then
		function tosticker(arg, data)
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_تصویر یافت نشد 😐❤️_', 1, 'md')
				end
			end
...
--------------------------------
if matches[1] == 'voice' or matches[1] == 'ساخت ویس' then
...
--------------------------------
	if matches[1]:lower() == 'short' or matches[1]:lower() == 'کوتاه کردن' then
...
--------------------------------
	if matches[1]:lower() == "photo" or matches[1]:lower() == "ساخت عکس" then 
		local eq = URL.escape(matches[2])
		local w = "500"
		local h = "500"
		local txtsize = "100"
...
💬 ماشین #حساب (جمع،تفریق،ضرب،تقسیم و غیره)

➖➖➖➖➖➖➖➖➖

*🔴 >>  tosticker [ reply ]*
🔴 >>   تبدیل به استیکر [ رپلی کردن ]
